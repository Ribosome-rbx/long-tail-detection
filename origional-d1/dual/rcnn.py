...
class DualRCNN_is(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        vis_period: int = 0,
        momentum: float = 0.99,
        image_rare_category_file=None,
        sample_rare_category_file=None,
    ):
        """
        Args:
        ...
        # record rare classes
        if "lvis" in image_rare_category_file and "lvis" in sample_rare_category_file:
            self.image_rare_categories = np.array(
                [int(x.rstrip()) - 1 for x in open(image_rare_category_file)]
            )
            self.sample_rare_categories = np.array(
                [int(x.rstrip()) - 1 for x in open(sample_rare_category_file)]
            )
        else:
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "image_rare_category_file" : cfg.MODEL.ROI_HEADS.IMG_RARE_CAT_FILE,
            "sample_rare_category_file" : cfg.MODEL.ROI_HEADS.SMP_RARE_CAT_FILE,
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        with torch.no_grad():  # no gradient to avg-model
            ...
            rare_images_list, rare_gt_instances, rare_img_idxs = self.extract_rare(images, gt_instances)
        ...
        _, detector_losses = self.roi_heads(new_images, new_features, proposals, gt_instances, self.sample_rare_categories, rare_img_idxs)
    ...
    def extract_rare(self, images, gt_instances):
        ...
        for image, instances in zip(images, gt_instances):
            if True in np.isin(instances.gt_classes.to('cpu'), self.image_rare_categories):
                rare_images_list.append(image)
                rare_gt_instances.append(instances)
                idxs.append(gt_instances.index(instances))
        return rare_images_list, rare_gt_instances, idxs
