...
class dualROIHeads(StandardROIHeads):
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances], len_rare = 0, rare_categories = None):
        ...
        if self.training:
            # memory bank
            augmented_proposals, box_features = self.use_memory_cache(box_features, proposals)
            proposal_list, feature_list = self.split_features(augmented_proposals, box_features)
        
        qk = box_features.unsqueeze(1)
        box_features = self.transformer(qk, qk) # transformer to be revised
        ...
        if self.training:
            ...
            if self.contrastive_branch:
                # transformer
                x_set, y_set, gt_classes = self.feed_transformer(proposal_list, feature_list)
                losses.update(self.contrastive_head(x_set, y_set, gt_classes)) 
            del box_features
    ...
    @torch.no_grad()
    def split_features(self, augmented_proposals = None, box_features = None):
        rare_features = []
        freq_features = []
        rare_proposals = []
        freq_proposals = []
        for p in augmented_proposals:
            gt_classes = p.gt_classes.cpu().numpy()
            fg_idxs = np.where(~np.isin(gt_classes, self.num_classes))
            gt_classes = gt_classes[fg_idxs]
            rare_idxs = np.where(np.isin(gt_classes, self.cache_categories))[0]
            freq_idxs = np.where(~np.isin(gt_classes, self.cache_categories))[0]
            if len(rare_idxs) > 0:
                rare_features.append(box_features[rare_idxs])
                rare_proposals.append(Instances.cat([p[int(i)] for i in rare_idxs]))
            if len(freq_idxs) > 0:
                freq_features.append(box_features[freq_idxs])
                freq_proposals.append(Instances.cat([p[int(i)] for i in freq_idxs]))
        if len(rare_proposals) > 0 and len(freq_proposals) > 0:
            proposal_list = rare_proposals + freq_proposals
            feature_list = [torch.cat(rare_features).unsqueeze(1), torch.cat(freq_features).unsqueeze(1)]
            return proposal_list, feature_list
        elif len(rare_proposals) == 0:            
            return freq_proposals, torch.cat(freq_features).unsqueeze(1)
        else:
            return rare_proposals, torch.cat(rare_features).unsqueeze(1)

    def feed_transformer(self, proposal_list = None, feature_list = None):
        '''
        proposal_list: [rare, freq]
        feature_list: [rare, freq]
        transformer(q: 12, k: 100) →  rare_1 12
        transformer(q: 100, k: 12) →  freq_1 100
        transformer(q: 12, k: 12) →  rare_2 12
        transformer(q: 100, k: 12) →  freq_2 100
        '''
        if torch.is_tensor(feature_list):
            feature_list = self.transformer(feature_list, feature_list)
            gt_classes = [prop.gt_classes for prop in proposal_list]
            return feature_list, feature_list, torch.cat(gt_classes)
        rare_1 = self.transformer(feature_list[0], feature_list[1])
        freq_1 = self.transformer(feature_list[1], feature_list[0])
        rare_2 = self.transformer(feature_list[0], feature_list[0])
        freq_2 = self.transformer(feature_list[1], feature_list[1])
        x_set = torch.cat([rare_1, freq_1])
        y_set = torch.cat([rare_2, freq_2])
        gt_classes = [prop.gt_classes for prop in proposal_list]
        return x_set, y_set, torch.cat(gt_classes)
