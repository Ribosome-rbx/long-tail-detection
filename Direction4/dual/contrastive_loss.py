...
class ContrastiveHead(nn.Module):
    ...
    def forward(self, x_set, y_set, gt_classes):
        # if len_rare == 0:
        ...
        x_feat = self.head(x_set)
        y_feat = self.head(y_set)
        x_feat_normalized = F.normalize(x_feat, dim=1)
        y_feat_normalized = F.normalize(y_feat, dim=1)
        contrastive_loss = self.criterion2(x_feat_normalized, gt_classes, y_feat_normalized, gt_classes)
