...
__all__ = ["DualRCNN_image"]


@META_ARCH_REGISTRY.register()
class DualRCNN_image(nn.Module):
    """
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        with torch.no_grad():  # no gradient to avg-model
            if self.avg_backbone == None:
                self.avg_backbone = copy.deepcopy(self.backbone)
            ...
            rare_images_list, rare_gt_instances = self.extract_rare(images, gt_instances)
        ...
        _, detector_losses = self.roi_heads(new_images, new_features, proposals, new_gt_instances)
    ...
    def extract_rare(self, images, gt_instances):
        ...
        return rare_images_list, rare_gt_instances
